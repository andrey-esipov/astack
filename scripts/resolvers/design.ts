...
function executablePath(dir: string, binary: string): string {
  return dir.startsWith('$') ? `${dir}/${binary}` : `$HOME${dir.replace(/^~/, '')}/${binary}`;
}
...
[ -z "$D" ] && D="${executablePath(ctx.paths.designDir, 'design')}"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
[ -z "$B" ] && B="${executablePath(ctx.paths.browseDir, 'browse')}"
if [ -x "$B" ]; then
  echo "BROWSE_READY: $B"
else
